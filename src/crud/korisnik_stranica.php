...
if(filter_input(INPUT_SERVER,'REQUEST_METHOD')== 'POST') {
    ...
    if($akcija < 4) {
        ...
        $stranica = filter_input(INPUT_POST, 'stranica');
    ...
    switch ($akcija){
        case 1://kreiranje
            $upit = "SELECT * FROM korisnikstranica WHERE stranica_id = $stranica AND korisnik_id = $korisnik AND vrijeme = $vrijeme";
            $rezultat = $baza->selectdb($upit);

            if($rezultat->num_rows){
                $poruka = "Taj zapis već postoji";
            }else{
                $upit = "INSERT INTO korisnikstranica VALUES ($korisnik, $stranica, $vrijeme)";
    ...
    if ($akcija == 5 && $pojam != ""){ // search
        ...
        $pojam = "%" . $pojam . "%";
        $upit = "SELECT * FROM korisnikstranica l JOIN stranica s ON l.stranica_id = s.id_stranica
                  JOIN korisnik k ON l.korisnik_id = k.id_korisnik
                  WHERE k.korisnicko_ime LIKE '$pojam' OR s.naziv_stranica LIKE '$pojam'";
